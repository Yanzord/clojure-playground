...
(db/adiciona-produtos-ou-altera-produtos! conn [dama])
(pprint (db/um-produto (d/db conn) (:produto/id dama)))

; update/insert ==> upsert
(db/adiciona-produtos-ou-altera-produtos! conn [(assoc dama :produto/slug "/jogo-de-dama")])
(pprint (db/um-produto (d/db conn) (:produto/id dama)))

(db/adiciona-produtos-ou-altera-produtos! conn [(assoc dama :produto/preco 150.5M)])
(pprint (db/um-produto (d/db conn) (:produto/id dama)))

; detectamos um problema que eh uma dificuldade, entender que updates sobrescreverao campos e operacoes anteriores

(defn atualiza-preco []
  (println "atualizando preco")
  (let [produto (db/um-produto (d/db conn) (:produto/id dama))
        produto (assoc produto :produto/preco 999M)]
    (db/adiciona-produtos-ou-altera-produtos! conn [produto])
...
(defn roda-transacoes [tx]
  (let [futuros (mapv #(future (%)) tx)]
    ...
    (pprint (pprint (db/um-produto (d/db conn) (:produto/id dama))))))
