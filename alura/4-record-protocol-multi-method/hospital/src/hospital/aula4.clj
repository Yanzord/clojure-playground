...
(let [particular (->PacienteParticular 15 "Yan" "02/04/1998" :normal)
      plano (->PacientePlanoDeSaude 15 "Yan" "02/04/1998" :normal [:raio-x :ultrassom])]
...
(let [particular (->PacienteParticular 15 "Yan" "02/04/1998" :urgente)
      plano (->PacientePlanoDeSaude 15 "Yan" "02/04/1998" :urgente [:raio-x :ultrassom])]
  (pprint (deve-assinar-pre-autorizacao? particular :raio-x 500))
  (pprint (deve-assinar-pre-autorizacao? particular :raio-x 40))
  (pprint (deve-assinar-pre-autorizacao? plano :raio-x 4099990))
  (pprint (deve-assinar-pre-autorizacao? plano :coleta-de-sangue 4099990)))
...
; nao se coloca multi no final, vou manter no mesmo arquivo para que facil para voce comparar as duas implementacoes
...
; exporando como funciona a funcao que define a estrategia de um defmulti
(defn minha-funcao [p]
  (println p)
  (class p))
...
; um pouco feeio pois estou misturando keyword e classe como chave
